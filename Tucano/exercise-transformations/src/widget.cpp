...
{        
	...
	modelmatrix.rotate (Eigen::AngleAxisf(M_PI * 0.5, Eigen::Vector3f::UnitZ()));
	mesh.setModelMatrix(modelmatrix);
	//mesh.modelMatrix()->rotate (Eigen::AngleAxisf(M_PI * 0.5, Eigen::Vector3f::UnitZ()));
}
 ...
 */
 ...
 {
	 Eigen::Affine3f viewmatrix = camera.getViewMatrix();
	 viewmatrix.rotate(Eigen::AngleAxisf(M_PI * 0.5, Eigen::Vector3f::UnitZ()));
	 viewmatrix.translate(Eigen::Vector3f(0.5, 0.0, 0.0));
...
{    
	mesh.modelMatrix()->rotate(Eigen::AngleAxisf(M_PI * 0.01, Eigen::Vector3f::UnitY()));
}
