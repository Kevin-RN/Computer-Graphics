...
public:
    ...
    {
		//Creates rotation matrix to rotare model 45 degrees around the z-axis (ex 3 pipelines)
		Eigen::Affine3f rotation_matrix = Eigen::Affine3f::Identity();
		rotation_matrix.rotate(Eigen::AngleAxisf(M_PI * 0.25, Eigen::Vector3f::UnitZ()));
		...
		shader.setUniform("rotationMatrix", rotation_matrix);
